...
main() {
  ...
  if [ "$_arg_echo" == "off" ] ; then
    ...
    docker run -it --rm -e PROGRAM=argbash-init -v "${workdir}":/work "${image}" "${_arg_output_file}" "${_arg_parameters[@]}"
    ...
    echo "docker run -it --rm -e PROGRAM=argbash-init -v" "${workdir}":/work "${image}" "${_arg_output_file}" "${_arg_parameters[@]}"
  fi
}
